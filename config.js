...
try {
    fs.readFileSync(path.join(app.getPath('userData'), 'calendar/person.json'), 'utf-8');
} catch (err) {
    fs.writeFileSync(path.join(app.getPath('userData'), 'calendar/person.json'), JSON.stringify([]));
}
